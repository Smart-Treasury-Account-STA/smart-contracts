...
| Adapter pinning at schedule approval | `intent_registry::ScheduledIntent.adapter`, pinned by `create_scheduled_payment`, dispatched from by `execute_scheduled_payment` |
| Timelocked adapter / guardian administration changes | `smart_account::propose_adapter_change`/`apply_adapter_change`/`cancel_adapter_change`; `recovery_manager::propose_remove_guardian`/`apply_guardian_removal`/`cancel_guardian_removal`; `recovery_manager::propose_threshold_change`/`apply_threshold_change`/`cancel_threshold_change` |
...
See `docs/V1_SCOPE.md` §3 for the specific signer/approval edge cases (mid-flight guardian removal, threshold changes after approval, replay across nonce/child-execution/cumulative-usage dimensions, recovery pull ordering, unmocked-auth negative tests, policy-version-bump/freeze/cancel/duplicate-recipient/shared-nonce/adapter-timelock cross-contract edge cases), §4 for the first round of findings, §5 for a second round focused on feature completeness against the architecture doc, and §6 for a third round (adapter pinning, and timelocking adapter/guardian administration changes). Full per-file breakdown:
...
- `smart_account`: init/status, full policy-engine-and-adapter-backed transfer payment, nonce replay rejection (and shared across transfer/split operations), asset-not-allowed propagation from `policy_engine`, paused/frozen rejection (and freeze has no direct unfreeze, **and pause/unpause reject a caller that isn't the owner**), split payment fan-out with independent per-recipient policy checks (fails closed if any one recipient isn't allowed, **if any one recipient's amount exceeds the cap even when others are fine, or if a recipient is duplicated**), scheduled-payment creation + relayer-triggered execution + **cancellation (end to end through intent_registry)**, **`execute_scheduled_payment` proven to use only canonical intent data**, **a policy-version bump between scheduling and execution blocks it closed**, **freezing blocks scheduled execution too**, **the adapter pinned at approval survives even a fully-applied reconfiguration to an address that isn't a valid adapter at all**, **creating a schedule before any adapter is configured fails fast**, **adapter changes are two-step and timelocked** (`propose_adapter_change`/`apply_adapter_change`/`cancel_adapter_change`, same too-early/cancel/permissionless-apply coverage as the guardian timelocks above), recovery pull happy path (owner replaced, freeze lifted, replay-guarded), recovery pull rejected pre-finalization, **a guardian can independently freeze the treasury with no owner involvement**, no-authorization-at-all rejection, instance/nonce TTL extension, and the composed OZ governance surface (`get_owner`, two-step `transfer_ownership`/`accept_ownership`, `add_context_rule`/`add_signer`/`remove_signer`). `ExecutionEntryPoint` (a generic arbitrary-contract-call passthrough OZ's `SmartAccount` composition offers by default) is deliberately *not* composed here — see `docs/SECURITY_REVIEW_STRICT.md` finding 13.

## Security Notes

This is a working V1, not yet a mainnet-ready system — see `docs/V1_SCOPE.md`'s "Not Yet Included in V1" and "Known, named residual risk" sections for the specific, named gaps still open (scoped session keys entirely absent; OZ's own documented weighted-threshold signer-divergence caveat; multisig/distributed authority for the admin/owner role itself), §4 for the first round of findings a dedicated security review found and fixed (a critical caller-supplied-data bug in scheduled execution, a missing minimum recovery delay, a missing guardian-activation delay, and the TTL/storage-archival production requirement that had no coverage at all), §5 for a second round (a scheduled payment that could never actually be cancelled, recovery that was unusable in exactly the scenario it exists for, no guardian-facing emergency freeze at all, unrejected duplicate recipients in a split), and §6 for a third, independently produced round: adapter pinning at schedule-approval time, and timelocking adapter reconfiguration and guardian administration so a single compromised key can no longer make those changes take effect immediately. All are stated explicitly rather than left for a reviewer to discover.
