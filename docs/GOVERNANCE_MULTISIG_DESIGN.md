...
- `contracts/governance_account` — composes OZ's `SmartAccount` exactly as sketched in §3's Option A, deliberately with no `Ownable`/`Pausable`/`ExecutionEntryPoint` (nothing here needs to *initiate* calls as itself). `initialize` mirrors `smart_account::initialize`'s own bootstrapping pattern (calls the raw `add_context_rule` storage primitive directly, since no signer exists yet to authorize through the trait entrypoint). 6 tests.
